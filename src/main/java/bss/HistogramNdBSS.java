...
public class HistogramNdBSS< T > implements Img< LongType >
 {
	 ...
	 private ArrayList<HistProgressListener> listeners = new ArrayList<>();
	 
	 private long lTotalSize = 0;
	 
	 private boolean bRAIList = false;
	  ...
	  */
	 public HistogramNdBSS( final List< BinMapper1d< T >> mappers )
	 {
	  ...
	  */
	 public HistogramNdBSS( final HistogramNdBSS< T > other )
	 {
		 final List< BinMapper1d< T >> mappersCopy = new ArrayList<>();
	  ...
	  *            The algorithms used to map values to bins
	  */
	 public HistogramNdBSS( final Iterable< List< T >> data, final List< BinMapper1d< T >> mappers )
	 {
		 this( mappers );
		 init( data );
	 }
	  ...
	  *            The algorithms used to map values to bins
	  */
	 public HistogramNdBSS( final List< Iterable< T >> data, final List< BinMapper1d< T >> mappers )
	 {
		 this( mappers );
		 init( data );
	 }
	 ...
	 /** Adds histogram calculation progress listener **/
	 public void addProgressListener( final HistProgressListener l)
	 {
		 listeners.add( l );
	 }
	 /**
	 ...
	 @Override
	 public HistogramNdBSS< T > copy()
	 {
		 return new HistogramNdBSS<>( this );
	 }
	 ...
	 private void init( final List< Iterable< T >> data )
	 {
		 ...
		 if(data.size() > 0 )
		 {
			 if(data.get( 0 ) instanceof RandomAccessibleInterval)
			 {
				 RandomAccessibleInterval<?> rai = ( RandomAccessibleInterval< ? > ) data.get( 0 );
				 bRAIList = true;
				 lTotalSize = Intervals.numElements( rai );
			 }
		 }
		 add( data );
	 }
	 ...
	 private void add( final List< Iterable< T >> data )
	 {
		 ...
		 {
			 modifyCountsRAI( data, incrementer );
		 }
	 }
	 ...
	 {
		 final List< T > vals = new ArrayList< >( mappers.size() );
		 final List< Iterator< T >> iters = new ArrayList<>();
	 ...
	 private void modifyCountsRAI( final List< Iterable< T >> data, final Counter counter )
	 {
		 ...
		 do
		 {
			 ...
			 if ( hasNext )
			 {
				 ...
				 count( vals, counter );
				 nPixelCount++;
				 int currentPercent = (int) ((nPixelCount * 100) / lTotalSize);
				 if (currentPercent > lastReportedPercent) 
				 {
					 for(HistProgressListener l:listeners )
					 {
						l.onProgress( nPixelCount, lTotalSize ); 
					 }
